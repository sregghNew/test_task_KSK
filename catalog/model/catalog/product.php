...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
        ...
        (
            "SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, 
                (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND 
                (
	...
	public function getProductSpecials($data = array()) {
        ...
        //custom
        //получаю список всех категорий только для элементов, которые являются "Special Offers"
    ...
    GROUP BY ps.product_id";
        ...
        $query = $this->db->query($sql);
        foreach ($query->rows as $result) {
            $category_data[$result['category_id']] = $result['name'];
        }
